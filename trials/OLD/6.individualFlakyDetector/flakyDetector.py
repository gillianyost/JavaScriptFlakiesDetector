...
while (counter < int(sys.argv[2])):
    ...
    if (flakyDetected):
        ...
        for testResult in flakyData['testResults']:
            for assertionResult in testResult['assertionResults']:
                if(testSuites[testResult['name'].split('package/')[1]][assertionResult['title']] == assertionResult["status"]):
                    if testResult['name'].split('package/')[1] in flakiesAll:
                        if testResult['name'].split('package/')[1] not in ODflaky:
                            numODFlakyTests = numODFlakyTests + 1
                            ODflaky.append(testResult['name'].split('package/')[1])
                            with open('../ODflakies.txt', 'a') as ODflakies:
                                ODflakies.write(sys.argv[1]+" : "+testResult['name'].split('package/')[1]+" : "+assertionResult['title']+"\n")
                else:
                    if testResult['name'].split('package/')[1] in flakiesAll:
                        if testResult['name'].split('package/')[1] not in NODflaky:
                            NODflaky.append(testResult['name'].split('package/')[1])
                            with open('../NODflakies.txt', 'a') as NODflakies:
                                NODflakies.write(sys.argv[1]+" : "+testResult['name'].split('package/')[1]+" : "+assertionResult['title']+"\n")
